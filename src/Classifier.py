...
class Classifier:
    def __init__(self, project, classifierPath, modelName):
        ...
        self.batchSize = 256    ##
        # self.batchSize = 128    ##
        #self.batchSize = 32    ##### Original val: 256
    ...
    def classify_resultExtract(self, data, data_img=None, data_code=None, tp='title'):
        if data_img != None and data_code != None:
            ...
            if tp=='title':
                prediction = torch.tensor([]).cuda()
                predict_val = torch.tensor([]).cuda()     # 삭제
                for idx, (samples_text, [image,label]) in enumerate(zip(loader_text, img_test_loader)):
                    x_img = image.cuda()
                    if idx+1 == len(loader_text):
                        _last = True
                    x_text = samples_text[0]
                    x_text = x_text.cuda()
                    p1 = self.model.predict(x_text, x_img=x_img)     # 삭제
                    #print(p1.shape)     # 삭제
                    predict_val = torch.cat([predict_val, p1])     # 삭제
                    p = torch.argmax(self.model.predict(x_text, x_img=x_img), dim=1)
                    prediction = torch.cat([prediction, p])
            else:
                prediction = self.model.predict(data, x_img=data_img)
            #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
            #print(predict_val)
            #print(predict_val.shape)
            #print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            ...
            if tp=='title':
                ...
                for idx, (samples_text, samples_code) in enumerate(zip(loader_text, loader_code)):
                    ...
                    x_code = samples_code[0]
                    x_code = x_code.cuda()
                    p1 = self.model.predict(x_text, x_code=x_code)     # 삭제
                    #print(p1.shape)     # 삭제
                    predict_val = torch.cat([predict_val, p1])     # 삭제
                    p = torch.argmax(self.model.predict(x_text, x_code=x_code), dim=1)
                    prediction = torch.cat([prediction, p])
            else:
                prediction = self.model.predict(data, x_code=data_code)
            #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
            #print(predict_val)
            #print(predict_val.shape)
            #print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
