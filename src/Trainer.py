...
class Trainer:
    def __init__(self, project, classifierPath, modelType, epoch, numCell, batchSize, maxLen, numClass, embeddingType):
        ...
        #self.additional_train = True
        #self.model = Model(modelType, numClass, numCell, maxLen, additional_train=self.additional_train, modelPath = self.modelPath, project=self.project)

    def fit(self, X=None, Y=None, X_img=None, X_code=None):
        if X_img != None and X_code != None:
            ...
            for e in range(1, self.epoch+1):
                #if self.additional_train==True:
                #    print("------ additional_train 11 ------")
                #    e = e+1000
                #else:
                #    print("------ First time 11 ------")
            ...
            if X != None:
                ...
                loader = DataLoader(dataset, batch_size=self.batchSize, shuffle=False)
                img_train_loader = DataLoader(X_img, batch_size=self.batchSize, shuffle=False, num_workers=1)
                for e in range(1, self.epoch+1):
                    print('{}/{} '.format(e, self.epoch), end='\r')
                    _last = False
                    ...
                    for idx, (samples, [image,label]) in enumerate(zip(loader, img_train_loader)):
                        ...
                        y = y.view(-1)
                        y = y.cuda()
                        self.model.fit(y=y, last=_last, x_img=x_img)
                    if e%50 == 0:
                        self.model.save('{}{}/{}-{}-{}.pt'.format(self.modelPath, self.project, self.modelType, self.embeddingType, e))
                print()
                # self.model.save('{}{}/{}-{}-{}.pt'.format(self.modelPath, self.project, self.modelType, self.embeddingType, e))
