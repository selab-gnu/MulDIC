...
class MAIN:
    ...
    def run(self):
        ...
        self._test_resultExtract(testX_text, testY_text, testX_code, project, modelPath, modelType)
    ...
    def _test_resultExtract(self, X, Y, X_code, project, modelPath, modelType, task=None, k=None):
        Y = Y.detach().cpu().numpy()
        #modelName = 'multimodal-EmbeddingLayer-1000.pt'
        modelName = 'multimodal_TC-EmbeddingLayer-900.pt'  # Check the Epoch Value
        classifier = Classifier(project, modelPath, modelName)
        
        predicts = classifier.classify_resultExtract(X, data_code=X_code)
        ...
        out_df.to_csv('resultData_extracted/MulDIC-TC_result.csv')
        print('======================== pred_out ========================')
        print(pred_out)
        print('\n======================== out_df ========================')
        print(out_df)
